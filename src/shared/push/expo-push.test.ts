...
describe('sendExpoPush', () => {
  it('envía título, texto y data a cada token, con el formato de Expo', async () => {
    ...
    await sendExpoPush(['ExponentPushToken[a]', 'ExponentPushToken[b]'], {
      title: 'Puntos',
    ...
    expect(JSON.parse(init.body)).toEqual([
      { to: 'ExponentPushToken[a]', title: 'Puntos', body: 'Recibiste 5 puntos', data: { orderId: 'o1' }, sound: 'default', channelId: 'default' },
      { to: 'ExponentPushToken[b]', title: 'Puntos', body: 'Recibiste 5 puntos', data: { orderId: 'o1' }, sound: 'default', channelId: 'default' },
    ]);
  });
  ...
  it('parte los envíos en lotes de 100', async () => {
    ...
    expect(JSON.parse(fetchFn.mock.calls[2][1].body)).toHaveLength(30);
  });
